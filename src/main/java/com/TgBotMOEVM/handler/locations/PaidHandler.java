...
public class PaidHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.PAID.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.PAID.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
