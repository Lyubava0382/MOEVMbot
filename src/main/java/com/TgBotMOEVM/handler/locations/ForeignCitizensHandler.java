...
public class ForeignCitizensHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.FOREIGN_CITIZENS.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.FOREIGN_CITIZENS.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
    ...
    public BotCommand getCommandObject() {
        return ButtonCommand.FOREIGN_CITIZENS;
    }
}
