...
public class TestWeekHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.TEST_WEEK.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.TEST_WEEK.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
    ...
    public BotCommand getCommandObject() {
        return ButtonCommand.TEST_WEEK;
    }
}
