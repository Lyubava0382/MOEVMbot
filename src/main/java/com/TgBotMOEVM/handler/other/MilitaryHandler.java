...
public class MilitaryHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.MILITARY.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.MILITARY.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
