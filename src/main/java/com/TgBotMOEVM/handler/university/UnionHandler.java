...
public class UnionHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.UNION.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.UNION.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
