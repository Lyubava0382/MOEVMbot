...
public class MOEVMHandler implements Handler {
    ...
    public List<BotApiMethod<?>> handle(Update update) {
        ...
        if (handbookRepository.findByCategory(ButtonCommand.MOEVM.getDescription()).isPresent()){
            responses.add(createMessage(chatId.toString(),
                    handbookRepository.findByCategory(ButtonCommand.MOEVM.getDescription()).get().getMessage()));
        }
        else{
            responses.add(createMessage(chatId.toString(),
